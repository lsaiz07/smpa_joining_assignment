...
Q1_data_set <- primary_dem_rep_clean %>%
  left_join(precinct_voting_locations, by = c("county" = "county_code", "election_district_precinct" = "precinct_code"))


Q1_primary_dem_rep_new <- Q1_data_set  %>% 
group_by(county, election_district_precinct) %>%
  filter(office_name == "Governor / Lt. Governor")%>% 
  mutate(votes= early_votes + election_night_votes + mail_in_ballot_1_votes + provisional_votes + mail_in_ballot_2_votes) %>%
  summarise(total_votes= sum(votes))

Q1_primary_dem_rep_new
...
Answer: For this question, I would ask what office recieved the highest number of votes and I would compare it with the other offices. 

Answer to my question: The House of Delegates was the office that recieved the highest ammount of votes. 

```{r}

Q1_primary_dem_rep_final <- Q1_data_set  %>% 
group_by(office_name) %>%
  mutate(votes= early_votes + election_night_votes + mail_in_ballot_1_votes + provisional_votes + mail_in_ballot_2_votes) %>%
  summarise(total_votes= sum(votes)) %>%
  arrange(desc(total_votes))

Q1_primary_dem_rep_final

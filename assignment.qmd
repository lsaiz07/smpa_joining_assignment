...
(primary_rep, by = c("County", "Election District - Precinct")) 

```{r}

primary_dem_rep <- bind_rows(primary_dem, primary_rep)
...
primary_dem_rep_clean <- primary_dem_rep %>%
...
```{r}
new_primary_dem_rep <- primary_dem_rep_clean %>%
...
```{r}
Q1_primary_dem_rep_clean <- primary_dem_rep_clean %>%
...
group_by(county, precinct_voting_locations, office_name == "Governor / Lt. Governor") %>%
  summarise(total_votes)


```{r}

Q1B_primary_dem_rep_clean <- Q1_primary_dem_rep_clean %>%
  group_by(county, precinct_voting_locations, office_name) %>%
  summarise(total_votes)
...
Q1_primary_dem_rep_clean %>%
  filter(total_votes == o)
...
Answer: For this question, I would ask: What office was the one that recieved the highest voter turnout and in what county was this the highest? 

To code this question, I would filter the office that recieved the highest turnout. I would then order them in descending order. Then I would group them by county name and see which one had the highest turnout. 
